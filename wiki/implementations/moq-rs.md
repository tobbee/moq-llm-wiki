...
- **draft-18**: `draft-18-dev` branch behind the runner-registered `moq-rs-draft-18` relay; the request-stream rework (PR #178, removes `MAX_REQUEST_ID`, moves requests to bidi streams) merged in the July-8 burst. **SUBSCRIBE_NAMESPACE support** ([PR #187](https://github.com/cloudflare/moq-rs/pull/187), itzmanish/Manish) merged to `main` July 19, and `draft-18-dev` was rebased onto `main` the same day to prep the Vienna Hackathon relay. **Relay draft-18 restoration** — [PR #211](https://github.com/cloudflare/moq-rs/pull/211) (Jacob/@nnazo, *"Restore relay work for draft-18 and port missing session layer,"* **+12164/−778**) merged **Aug 18**, bringing the **relay** role — which had lagged on draft-14/16 while the client tracked draft-18 — back onto draft-18 and porting the missing session layer, with unit-test coverage from [PR #156](https://github.com/cloudflare/moq-rs/pull/156) (+950/−71). The queued release-please round ([PR #212](https://github.com/cloudflare/moq-rs/pull/212)) **merged Aug-29**, cutting the `moq-relay-ietf` **v0.7.26** train (see *Latest release* below). Follow-on draft-18 relay work landed Aug-28: [PR #220](https://github.com/cloudflare/moq-rs/pull/220) *"treat peer-forwarded PUBLISH_NAMESPACE as discovery-only"* (+430/−14), [PR #222](https://github.com/cloudflare/moq-rs/pull/222) *"Add rendezvous timeout support"* (nnazo, +2010/−158), [PR #217](https://github.com/cloudflare/moq-rs/pull/217) *"correlate logs by MoQ session"* (itzmanish, +1675/−192), and [PR #221](https://github.com/cloudflare/moq-rs/pull/221) TrackName test fixes. **Pre-hackathon draft-18 conformance push (Aug 31 → Sep 1)**: driven by [[alan-frindell|afrind]]'s [[moxygen]] conformance run (see [[interop-runner]]), [[mike-english]] landed three large fixes on `draft-18-dev` — [PR #224](https://github.com/cloudflare/moq-rs/pull/224) *"support draft-18 subgroup modifiers"* (+1568/−165, Aug-31 02:39 UTC), [PR #225](https://github.com/cloudflare/moq-rs/pull/225) *"preserve draft-18 datagram semantics"* (+1141/−575), and [PR #226](https://github.com/cloudflare/moq-rs/pull/226) *"await publish terminal delivery"* (+1763/−293, Sep-01), and [PR #227](https://github.com/cloudflare/moq-rs/pull/227) *"preserve ordered rendezvous teardown"* (nnazo, **+4308/−738**, Sep-01). After #224 the moxygen conformance run showed the relay **passing Section 1 tests 1–3** on both QUIC and WebTransport (up from failing before SUBSCRIBE completed on Aug-30); English then **landed a datagram fix** (Aug-31 23:35) and afrind's **Sep-1 re-run reported the relay passing conformance Sections 1 *and* 2**, with **Section 3 now the front** (*"Section 3 had some troubles, I'm checking"*). **At the Sep-2 draft-18 interop hackathon the relay reached Sections 1–7 passing — 43/59 on both QUIC and WebTransport** (afrind: *"CF passes section 1-7"*), failing only Section 8 (PUBLISH) and Section 10 (Joining FETCH), after four Sep-2 fixes: [PR #228](https://github.com/cloudflare/moq-rs/pull/228) *"preserve subgroup fanout"*, [PR #229](https://github.com/cloudflare/moq-rs/pull/229) *"preserve subgroup object status"*, [PR #230](https://github.com/cloudflare/moq-rs/pull/230) (rendezvous-timeout interop scenario), and [PR #231](https://github.com/cloudflare/moq-rs/pull/231) *"reject duplicate namespace requests locally"*. **Sep-3** added **inbound `SUBSCRIBE_TRACKS` support** ([PR #232](https://github.com/cloudflare/moq-rs/pull/232), +2004/−39) and a follow-on surfacing SUBSCRIBE_OK params / PUBLISH_DONE details / clean subgroup close ([PR #233](https://github.com/cloudflare/moq-rs/pull/233), +263/−19) — both [[mike-english]] on `draft-18-dev`. (Separately, the hackathon sweep diagnosed a moq-rs `SUBSCRIBE_OK` wire bug — its `KeyValuePairs::encode` writes a spurious "Number of Extensions" count varint before the trailing Track Extensions, which draft-16 Fig. 12 defines as a bare count-less KVP run; symmetric decode means moq-rs interops with itself but not with moxygen. Diagnosed by [[giovanni-marzot|Marzot]], headed upstream; see [[discussions-2026-09]].) The umbrella draft-18 PR [#176](https://github.com/cloudflare/moq-rs/pull/176) stays OPEN. **No new release — v0.7.26 stands**
...
# Recent Highlights (as of August 2026)
...
- **draft-16 rewrite + PUBLISH support.** Manish (@itzmanish) closed his 155-day-old [PR #131](https://github.com/cloudflare/moq-rs/pull/131) and reopened it as [PR #170](https://github.com/cloudflare/moq-rs/pull/170) ("[Rewrite] Draft-16 migration"), the underlying draft-16 baseline (merged July 8). Its large follow-on [PR #181](https://github.com/cloudflare/moq-rs/pull/181) ("Publish message support", +4049/−624) merged July 9, adding native PUBLISH-message handling and triggering a `moq-relay-ietf` v0.7.22 / `moq-pub` v0.9.0 release round.
...
- **draft-18 relay hardening + v0.7.26 (Aug-28/29).** After ~7 quiet days the relay woke up again for pre-hackathon prep: [PR #220](https://github.com/cloudflare/moq-rs/pull/220) makes a **peer-forwarded PUBLISH_NAMESPACE discovery-only** (a relay-mesh clustering behavior), [PR #222](https://github.com/cloudflare/moq-rs/pull/222) adds **rendezvous-timeout support** (Jacob/@nnazo), and [PR #217](https://github.com/cloudflare/moq-rs/pull/217) adds **session-correlated log observability** (Manish/@itzmanish). The queued release-please round ([PR #212](https://github.com/cloudflare/moq-rs/pull/212)) then merged, cutting the **`moq-relay-ietf` v0.7.26** train (Aug-29) — the first tagged release since July 31.
