...
- **Relay upstream-retention fix.** [PR #196](https://github.com/cloudflare/moq-rs/pull/196) ([[mike-english]], +1740/−150, merged July 30) **releases upstream subscriptions for idle cached tracks and stops FIN-ing subgroup streams mid-object** — fixing the [issue #191](https://github.com/cloudflare/moq-rs/issues/191) bug where the relay held an upstream subscription after the last downstream subscriber left (the same relay-lifecycle class as [[moqtail]] [issue #332](https://github.com/moqtail/moqtail/issues/332)). A durable correctness gain on the recurring post-Vienna relay-resource-lifecycle front.
