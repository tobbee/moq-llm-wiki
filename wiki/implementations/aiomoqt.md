...
- **Draft 14 / 16 / 18** general-purpose MOQ client + server, with ALPN-based negotiation (`moq-00` for draft-14, `moqt-16` for draft-16, `moqt-18` for draft-18)
- **Latest release: v0.11.1 (2026-09-22 00:50 UTC)** — tag [`0.11.1`](https://github.com/gmarzot/aiomoqt/releases/tag/0.11.1), from [PR #38](https://github.com/gmarzot/aiomoqt/pull/38) (+2,351/−333 across 45 files). The headline is **relay FETCH plus namespace discovery**: the gap [[aman-sharma|Aman Sharma]] hit on Sep-17 (*"your relay doesn't handle FETCH"*) and [[will-law|Will Law]] answered on Sep-18 with the design rule that **a cacheless relay should issue its own upstream FETCH against the original publisher**. Also in the release: **`PUBLISH_DONE` ends a subscription, not the session** (with `register_publish_done_handler()` / `SubscribedTrack(on_done=…)`); the **default FETCH handler rejects with `NOT_SUPPORTED`** instead of sending a `FETCH_OK` it never follows with a data stream (§10.13); object timestamps move to the **loc-04 `TIMESTAMP` (0x10, wall-clock µs)** from the private `0x20`, with receivers accepting both; and `--draft` accepts `draft-18` / `d18` / `moqt-18` spellings. Sharma's Sep-21 verification: *"The FETCH fixes all check out, and SUBSCRIBE_NAMESPACE now delivers NAMESPACE"* — with one open gap, **observers never receive `NAMESPACE_DONE`**, on either namespace withdrawal or publisher session close. It published *after* the Sep-22 interop nightly started, so the matrix cannot reflect it before the Sep-23 cut.
- **Latest release: v0.11.0 (final, 2026-09-17)** — tag [`0.11.0`](https://github.com/gmarzot/aiomoqt/releases/tag/0.11.0) (published Sep-17 14:13 UTC via [PR #36](https://github.com/gmarzot/aiomoqt/pull/36)), shipping alongside **aiopquic v0.4.0** ([PyPI](https://pypi.org/project/aiopquic/)). This is the first *final* 0.11.0 after a long rc chain (rc3 Sep-3 → rc6 Sep-13). Announced on `#moq` (Marzot, Sep-17) as *"Drafts 14, 16, 18 general purpose MOQ client/server package … numerous performance improvements, conformance fixes and added media pipeline support w/ MSF/CMSF LOC/CMAF/TS."* It finally ships the long-in-flight **SUBSCRIBE_OK `vi64` decode fix** (see below) in a tagged release, plus a new **media pipeline** carrying LOC/CMAF/TS payloads via MSF/CMSF. Prior stable: **v0.10.6** (~July 8, 2026; PyPI [`aiomoqt`](https://pypi.org/project/aiomoqt/))
- **v0.11.0 interop, Sep-17**: [[steven-riedl|Steven Riedl]] (Pluto TV) ran the ghcr `latest` (14:13Z) build through the interop-runner harness against the `stitcher-moq` / moq-relay endpoints and got **6/6 on raw QUIC and 6/6 on WebTransport** (probe pins draft-16; `DRAFT=18` also 6/6) — tests 4 and 6 pass via aiomoqt's COMPAT path around moq-relay's non-spec 404 (a moq-dev-side quirk, not aiomoqt). Since the nightly pulls `latest`, the first runner matrix with 0.11 across the board is the Sep-18 cut. Riedl also flagged an **entrypoint draft-pin bug**: `DRAFT=draft-18` (the runner-registry spelling) makes `moq_interop_client` exit 2 (`invalid parse_draft_spec value: 'draft-18'`, no TAP) and `MOQT_DRAFT=draft-18` is silently ignored — only `DRAFT=18` / `--draft 18` work; accepting `draft-NN` would make the runner's injected `DRAFT` safe. Separately [[aman-sharma|Aman Sharma]] noted aiomoqt's **`ersatz-relay` doesn't handle FETCH** and SUBSCRIBE_NAMESPACE may be incomplete — Marzot's reply reiterated it is *"not a real relay"* (no cache; a test vehicle for exercising more APIs/scenarios), extensible on request.
...
- **draft-18 SUBSCRIBE_OK parse bug (found 2026-08-13; fixed in v0.11.0)**: v0.10.6 decodes the SUBSCRIBE_OK Track-Properties block with **RFC 9000 varints instead of the LOC-style `vi64`**, so it runs off the end of the message whenever a property value is ≥ 64 (e.g. `TIMESCALE=1000`, encoded `83 e8`) — surfacing as *"truncated trailing extensions block."* Root-caused by Giovanni Marzot after Steven Riedl (Pluto TV) hit it subscribing to a [[moq-dev]] relay (whose SUBSCRIBE_OK is spec-correct); the encode/decode fix **shipped in aiomoqt v0.11.0 (Sep-17) alongside aiopquic v0.4.0**. Workaround on v0.10.6: `MOQTMessage._tolerate_trailing_extensions = True` completes the subscribe but skips the property.
