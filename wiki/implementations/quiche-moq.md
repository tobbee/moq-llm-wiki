...
- **FETCH → a bidirectional stream** (Sep 16, 2026) — commit [`bc4e3dd9`](https://github.com/google/quiche/commit/bc4e3dd9be) *"Move FETCH to a bidi stream as part of draft-18 update"* (22:08 UTC) continues the draft-18 wire migration by moving FETCH off the control stream onto its own bidirectional stream (the same treatment SUBSCRIBE, TRACK_STATUS and PUBLISH_NAMESPACE already got), and **removes `request_id` from FETCH_OK / REQUEST_ERROR** to align the callback structs with the wire messages (marked *"Not in production"*). The **first moqt-module commit since Sep-4**, ending a ~12-day quiet stretch.
