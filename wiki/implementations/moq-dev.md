...
- **Compression experiment** (group-scoped DEFLATE, extracted into a `moq-flate` / `@moq/flate` crate) — the code side of Luke's June "MoQ + Compression" list thread. After a period of being reconsidered rather than linearly shipped, it is now being **written up as an in-repo IETF draft** ([#3715](https://github.com/moq-dev/moq/pull/3715) *"draft(flate): group-scoped DEFLATE as an IETF draft"*, Sep-17) — the same author-in-repo path the `moq-archive` and `moq-cluster` drafts took.
...
- **In-band security became a sustained, two-front workstream** (Sep 2026, still in early churn). **(1) End-to-end object encryption**: a **`moq-e2ee-01` profile with *shared* test vectors** was drafted in-repo ([#3629](https://github.com/moq-dev/moq/pull/3629), Sep-12) and the **Rust core** landed ([#3665](https://github.com/moq-dev/moq/pull/3665), Sep-14, +3381/−23), then the profile was **reset** — the TypeScript `-01` core was closed unmerged ([#3663](https://github.com/moq-dev/moq/pull/3663)) and a **`moq-e2ee-00` redesign with a *path epoch*** merged ([#3690](https://github.com/moq-dev/moq/pull/3690), Sep-15); the Rust `-01` core stays in `main` but the profile is being reworked, not linearly extended. **(2) Session auth via a `moq-auth` / Common Access Token reference server**: the `moq-auth` + `@moq/auth` contract/token crates and a **`moq auth serve` reference server** landed on `main` ([#3684](https://github.com/moq-dev/moq/pull/3684) +3839/−3446, [#3686](https://github.com/moq-dev/moq/pull/3686), [#3688](https://github.com/moq-dev/moq/pull/3688) — all merged Sep-15), admitting every session through a **moq-auth lease** — the same in-band CAT/CWT theme as [[moq-rs]]'s still-OPEN CAT [#235](https://github.com/cloudflare/moq-rs/pull/235) and [[moq-transport]] [#1927](https://github.com/moq-wg/moq-transport/pull/1927). A genuinely new security direction beyond transport/media; neither front is in a tagged release yet.
