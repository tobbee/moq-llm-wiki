...
**Draft support**: **draft-16 on `main`, with a draft-18 alignment sprint underway** (July–August 2026) — ALPN bumped to `moqt-18`, Extension Headers renamed to Properties, single unified SETUP (CLIENT_SETUP/SERVER_SETUP collapsed), split delivery timeouts, control-message-type table + LOC property ids renumbered to draft-18, and draft-18 constants + varint conformance vectors added; the sprint reached the Rust side in July and the **`moqtail-ts` (TypeScript) side began landing the same wire changes Aug 10** ([Kerem Bekmez](https://github.com/kerembkmz)), working down the ~20-issue moqtail-ts API-break backlog (draft-14 docs removed May 4 2026)
