...
> **2026-07-03**: **A WG-side contributor sends a FETCH-priority test helper — sharmafb opens [PR #213](https://github.com/moqtail/moqtail/pull/213) (July 2 22:19).** *"Helper to validate different priority in same subgroup in FETCH response"* — a validation helper checking that objects with **different priorities within the same subgroup** are handled correctly in a FETCH response. The notable signal is the author: **sharmafb (Aman Sharma)** is a **[[moq-transport]] editorial contributor** (the June-30 ALPN/`SUBSCRIBE_TRACKS`/skip-unknown-property polish PRs), so a WG-side reviewer contributing test tooling to the moqtail codebase is cross-pollination between the spec-editing and implementation sides. Follows June-30/July-1's on-the-wire correctness fixes (KVP v16-delta #208, subgroup-header bitmask dispatch #211). See [[moq-transport]], [[discussions-2026-07]].
>
