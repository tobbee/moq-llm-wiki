...
| `moq-go` | **Go implementation — not yet covered by a wiki page** | **19** | Ahead of the draft-18 target; since Aug-19 pairs only with itself (6/6). Was the sole "ahead" endpoint until `stitcher-moq` declared **draft-20** Sep-11 (see below) |
...
| `moqt-nr` | **Nokia relay ([[yu-you|Yu You]]) — not yet covered by a wiki page** | 18 → **18 / 19** | `moqt.nokiaresearch.com:4443`; **registered for draft-19 Sep-3** (runner `add moqt-19` commit + [#124](https://github.com/englishm/moq-interop-runner/pull/124) `moqt://` WT endpoint) — first draft-19 relay registration. **Cleared the moxygen conformance suite Sep-7** ([[yu-you|Yu You]]: *"moxygen conformance test passed"*) after the Sep-2–7 sweep fixed, in order: duplicate-object + FETCH-inclusive (Sep-4), then the publisher-priority mismatch (0 → 128 → correct) and the FORWARD=0-still-forwards bug. **Sweep resumed Sep-8**: [[aman-sharma|Aman Sharma]] found a **Joining Fetch against a Forward State 0 subscription wrongly accepted** (should be `REQUEST_ERROR`/`INVALID_RANGE` per §10.12.2) — "cleared conformance" stays a moving bar as the moqtest suite deepens. **Sep-9**: [[yu-you|Yu You]] opted **not to chase** that draft-18 joining-fetch fix — *"we are leaving the joining fetch impl. behind and moving on to the new v20/21 as the new fill fetch is coming"* — and floated hosting **v18 + v20/21** at the Seattle interop |
...
| `stitcher-moq` | **Pluto TV / Paramount relay — not yet covered by a wiki page** ([[steven-riedl|Steven Riedl]]) | 18 → **20** | Relay; **enrolled 2026-08-27** (runner [PR #112](https://github.com/englishm/moq-interop-runner/pull/112)), expanding the matrix to 351 cells. **Declared draft-20 support Sep-11** ([PR #126](https://github.com/englishm/moq-interop-runner/pull/126)) — the **first draft-20 endpoint in the runner** (ahead of the draft-18 nightly target), adding 3 "ahead" cells that show as skipped on the Sep-12 cut |
...
> **Coverage gap**: `moq-go`, `moq5`, `moqt-nr`, and (since Aug-27) `stitcher-moq` are registered and running but have **no wiki page yet**. `moq-go` (draft-19) and, since Sep-11, `stitcher-moq` (draft-20 — the **first draft-20 endpoint**, Pluto TV / Paramount's public relay) are the runner's implementations ahead of the draft-18 target.

**Registration history** (PRs on [englishm/moq-interop-runner](https://github.com/englishm/moq-interop-runner)): `moqx` relay #59 (Apr 11); then a May-13 batch — `mlmtest`/moqlivemock #63, `moqx` client #66, `aiomoqt` #67, and Nokia Docker `RELAY_URL` support #65 enabling Nokia's in-house v17 relay. **Merged Aug-27**: [#112](https://github.com/englishm/moq-interop-runner/pull/112) registering Pluto TV / Paramount's `stitcher-moq` public relay endpoints (Steven Riedl, 14:01 UTC — this expanded the matrix to 351 cells on the 14:16 cut) and [#111](https://github.com/englishm/moq-interop-runner/pull/111) documenting the moxygen/moqx draft-18 client ALPN limitation (14:02 UTC). **Merged Sep-1**: [#116](https://github.com/englishm/moq-interop-runner/pull/116) adding a **`stitcher-moq` test client** (Steven Riedl — the Pluto TV / Paramount endpoint now runs as client *and* relay), [#117](https://github.com/englishm/moq-interop-runner/pull/117) advertising a **draft-20 (`moqt-20`) ALPN for `moq-go`**, [#118](https://github.com/englishm/moq-interop-runner/pull/118) (image-rebuild workflow), and [#114](https://github.com/englishm/moq-interop-runner/pull/114) (`moqlivemock` → draft-18). **Merged Sep-2 (hackathon)**: [#119](https://github.com/englishm/moq-interop-runner/pull/119) adding **`moq-playa`** (OpenMOQ's `@moqt` TypeScript draft-18 client, `ghcr.io/openmoq/moq-playa-interop-client`), [#120](https://github.com/englishm/moq-interop-runner/pull/120) adding the **`mlmrel`** relay role ([[moqlivemock]]), [#121](https://github.com/englishm/moq-interop-runner/pull/121) putting **[[moqtail]] on draft-18** (the live `relay18.moqtail.dev`), [#122](https://github.com/englishm/moq-interop-runner/pull/122) bumping the embedded **moq.dev client to 0.19**, and [#123](https://github.com/englishm/moq-interop-runner/pull/123) spec'ing a **RENDEZVOUS_TIMEOUT test**. **Merged Sep-3**: an `add moqt-19` commit registering **Nokia's `moqt-nr` relay for draft-19** (its `implementations.json` entry goes `["draft-18"]` → `["draft-18","draft-19"]`, "Support native QUIC and WebTransport") — the **first draft-19 relay registration** — plus [#124](https://github.com/englishm/moq-interop-runner/pull/124) (Yu You, switching the WebTransport endpoint to a `moqt://` URL). **Merged Sep-5**: [#125](https://github.com/englishm/moq-interop-runner/pull/125) (MOQtail relay URL update). **Merged Sep-11**: [#115](https://github.com/englishm/moq-interop-runner/pull/115) *"docs: define interop reporting and test lifecycle"* (the neutral data-plane contract / reporting spec) and [**#126**](https://github.com/englishm/moq-interop-runner/pull/126) *"stitcher-moq: declare draft-20 support"* (Steven Riedl, 16:37 UTC) — the **first draft-20 endpoint**, which grew the matrix to 437 cells (ahead 19 → 22) on the Sep-12 cut. **Open now**: [#110](https://github.com/englishm/moq-interop-runner/pull/110) registering the `kvick` relay (Mattias Bergström, since Aug-15); and the conformance-test integration [#103](https://github.com/englishm/moq-interop-runner/pull/103).
...
**The Seattle interop target is now draft-20 (interim-23 decision, Sep-8; minutes posted Sep-9).** The [[interim-meetings|interim-2026-moq-21]] minutes (Aug-14) had named draft-22 the "next official interop target," but **[[interim-meetings|interim-2026-moq-23]] revised this**: the chairs/editors debated draft-18 vs draft-20 for the **Oct 12–13 Seattle hybrid interop**, and after a session poll (*"I will be able to support draft-20 in Seattle"* — **Yes 7 / No 5 / No opinion 4** of 21) agreed *"the formal interop target will be Draft 20."* [[ian-swett|Ian Swett]] and [[alan-frindell|afrind]] argued for testing **Fill Fetch** (which landed in draft-20) and filters; [[suhas-nandakumar|Suhas]], [[martin-duke|Duke]], and [[zafer-gurel|Zafer Gürel]] noted their codebases are still on draft-18, so those may **interop best-effort**. Crucially, **no further wire-breaking design changes will be merged into the interop target** — a wire-freeze. This is coherent with the draft-21/22 plan (see [[moq-transport]]): **draft-21 is a pure structural move and draft-22 the editorial cleanup, both wire-identical to draft-20**, so "target draft-20" means freezing the draft-20 *wire* while the document reorganizes around it. (Aside: the Aug-14 minutes called draft-20 "a purely-editorial cut"; the published draft-20 is **not** — it removes Joining FETCH and respecs the datagram/subgroup type fields.) **The automated nightly runner still runs draft-18 for now**; the shift to draft-20 is the Seattle-event target and has not yet propagated to the nightly matrix.

This draft-18→draft-20 pivot is already visible in implementations: on Slack (Sep-9) [[yu-you|Nokia]] said it is *"leaving the joining fetch impl. behind and moving on to the new v20/21 as the new fill fetch is coming"* and floated hosting **two versions (v18 + v20/21)** at Seattle, and afrind said the conformance script *"will hopefully support d20 / fill fetch by then"*; [[moq-dev]] added draft-21 wire support (`moqt-21`) the same day the draft was cut. See [[discussions-2026-09]].

# Current standing

**Latest cut: [2026-09-12 00:24:46 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-12_002446/report.html) — 437 cells / 199 pass / 235 fail / 3 skip** (45.5% pass; **at-target draft-18 298 · ahead 22 · behind 117**). **The matrix grew +3 cells (434 → 437), all in the "ahead" band (ahead 19 → 22) and all skipped (skip 0 → 3)** — the first roster change in weeks, driven by **`stitcher-moq` declaring draft-20 support** (Pluto TV / Paramount, runner [PR #126](https://github.com/englishm/moq-interop-runner/pull/126), merged Sep-11 16:37 UTC — the **first draft-20 endpoint in the runner**, aligning with the Seattle draft-20 target). Within the unchanged draft-18 band, one cell flipped **pass → fail (200 → 199)**, an ordinary within-band flake. **The nightly runner itself still targets draft-18**, even though [[interim-meetings|interim-2026-moq-23]] (Sep-8, minutes posted Sep-9) set the **Seattle Oct interop target to draft-20** (see [Current Target](#current-target)) — stitcher-moq's draft-20 declaration is the first sign of the roster moving toward that target ahead of the nightly matrix.

The prior **Sep-11 nightly: [2026-09-11 00:26:18 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-11_002618/report.html) — 434 cells / 200 pass / 234 fail / 0 skip** (46.1% pass; **at-target draft-18 298 · ahead 19 · behind 117**). **Pass +1 (199 → 200), fail −1**, with the **at-target / ahead / behind split byte-identical (298 / 19 / 117)** to the Sep-10 cut — a small bounce back after the Sep-10 −8, cells flapping within their existing version band, no reclassification or roster change (a fifth straight within-band flake).
...
The prior **Sep-8 nightly: [2026-09-08 00:26:55 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-08_002655/report.html) — 434 cells / 208 pass / 226 fail / 0 skip** (47.9% pass; **at-target draft-18 298 · ahead 19 · behind 117**). **Pass −2 (210 → 208), fail +2**, with the **at-target / ahead / behind split byte-identical (298 / 19 / 117)** to the Sep-7 cut — another within-band fail flip, no reclassification or roster change. It ran ~00:27 UTC, **before** both the Sep-8 [[interim-meetings|interim-2026-moq-23]] meeting and the **evening resumption of the relay-conformance sweep** ([[aman-sharma|Aman Sharma]]'s new [[imquic]] FETCH-object-ordering bug + a Nokia `moqt-nr` Joining-Fetch-on-Forward-State-0 bug), so none of those show here. Still targets **draft-18**.

The prior **Sep-7 nightly: [2026-09-07 00:29:12 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-07_002912/report.html) — 434 cells / 210 pass / 224 fail / 0 skip** (48.4% pass; **at-target draft-18 298 · ahead 19 · behind 117**). **Pass −1 (211 → 210), fail +1**, with the **at-target / ahead / behind split byte-identical (298 / 19 / 117)** to the Sep-6 cut — a single within-band fail flip, no reclassification or roster change. (Nokia's `moqt-nr` clearing the moxygen conformance suite Sep-7 15:09 CEST landed after this ~00:29 UTC cut, so its effect, if any, shows on the next nightly.) Still targets **draft-18**.

The prior **Sep-6 nightly: [2026-09-06 00:29:32 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-06_002932/report.html) — 434 cells / 211 pass / 223 fail / 0 skip** (48.6% pass; **at-target draft-18 298 · ahead 19 · behind 117**). **Pass +4 (207 → 211, a new high), fail −4**, with the **at-target / ahead / behind split byte-identical (298 / 19 / 117)** to the Sep-5 cut — so all four new passes are cells flipping fail→pass *within* their existing version band, more of the within-band tail of the Day-3 relay-sweep fixes reaching the nightly rather than any new endpoint or reclassification. Still targets **draft-18**.
...
The prior day's **[2026-09-01 00:31:10 UTC](https://englishm.github.io/moq-interop-runner/results/2026-09-01_003110/report.html) cut — 347 cells / 126 pass / 210 fail / 11 skip** (36.3% pass; **at-target draft-18 231 · ahead 3 · behind 113**). The **matrix trimmed 4 cells** (351 → 347, skip +1 to 11), but the composition **shifted markedly toward the target**: **at-target draft-18 rose +21 (210 → 231)** while **behind fell −25 (138 → 113)** — roughly two dozen cells reclassifying from an older draft onto draft-18 as implementations **converge ahead of the [[interim-meetings|Sep-2 virtual interop hackathon]]** (registered draft-18 changes: runner [#114](https://github.com/englishm/moq-interop-runner/pull/114) put `moqlivemock` on draft-18, [#116](https://github.com/englishm/moq-interop-runner/pull/116) added a `stitcher-moq` **client**, [#117](https://github.com/englishm/moq-interop-runner/pull/117) gave `moq-go` a **draft-20 `moqt-20` ALPN**). Pass ticked **+1 to 126**; the relay roster now lists `moq-rs`, `moq-rs-draft-14` and **`moq-rs-draft-18`** as distinct endpoints, reflecting the conformance split. Still targets **draft-18**.

The prior day posted **two Aug-31 cuts** (both on the 351-cell matrix): [00:29:26 UTC](https://englishm.github.io/moq-interop-runner/results/2026-08-31_002926/report.html) — **351 / 122 / 219 / 10** — then [04:12:16 UTC](https://englishm.github.io/moq-interop-runner/results/2026-08-31_041216/report.html) — **351 / 125 / 216 / 10** (at-target 210 · 3 · 138), so pass climbed **116 (Aug-30) → 122 → 125 → 126 (Sep-1)** — a **+10 recovery** off the Aug-30 low, lining up with the **draft-18 conformance work landing on the relays** ([[moq-rs|cloudflare/moq-rs]] merged its *"support draft-18 subgroup modifiers"* fix [#224](https://github.com/cloudflare/moq-rs/pull/224) at 02:39 UTC between the two Aug-31 cuts). (See [Conformance sweep](#conformance-sweep-live-relays) below — afrind's moxygen conformance run drove the moq-rs fixes; moq-rs went from failing before SUBSCRIBE on Aug-30 to passing **conformance Sections 1 and 2** by Sep-1.)
...
|---|---|---|
| [[moq-rs]] (draft-18) | SUBSCRIBE reaches the server and SubscribeOk is sent, the relay opens subgroup stream id=6 — then **resets it ~39 ms later, error=0**, so no objects flow | **Passing conformance Sections 1 and 2** (Sep-1) after [[mike-english]] landed conformance fixes (incl. a datagram fix on `draft-18-dev`) — up from Section 1 tests 1–3 with Test 4 (DATAGRAM) failing on Aug-31; **Section 3 now the front** (afrind: *"Section 3 had some troubles, I'm checking"*) |
...
**Sep-2 hackathon results** — the conformance loop (afrind runs [[moxygen]]'s `moq-test` script against each live relay, posts the per-section matrix, the author fixes and redeploys) turned the Aug-30 rough picture around. The suite now runs **10 sections** (1 Basic Forwarding Preferences · 2 Object/Group Counts · 3 Object Sizes · 4 Group/Object Increments · 5 End-of-Group Markers · 6 Extensions · 7 Complex Scenarios · 8 PUBLISH · 9 FETCH Ranges · 10 Joining FETCH), with **Sections 9–10 added Sep-1**. End-of-day scores (Slack `#moq`):

| Relay | Sep-2 result | Movement |
|---|---|---|
| [[moxygen]] (Meta) | **74/76 quic · 75/76 WT** | Recovered from a self-inflicted **58/76 quic · 69/76 WT** over a real cross-country network (afrind: *"I failed my own test suite … How embarrassing"*) — network-timing bugs in the harness itself |
| [[moq-rs]] (Cloudflare) | **43/59 — Sections 1–7** ✅ | From Sections 1&2 (Sep-1); four Sep-2 fixes ([#228](https://github.com/cloudflare/moq-rs/pull/228)–[#231](https://github.com/cloudflare/moq-rs/pull/231)). Fails Section 8 (PUBLISH), Section 10 (Joining FETCH) |
| [[moqtail]] (OzU) | **53/59** | From **4/17** at deploy — a live bug-fix loop (afrind + [[aman-sharma|Aman Sharma]] found, [[zafer-gurel|Gürel]] fixed): PUBLISH_DONE held until data streams arrive ([#382](https://github.com/moqtail/moqtail/pull/382)), a concurrent-PUBLISH track-record race ([#381](https://github.com/moqtail/moqtail/pull/381)/[#383](https://github.com/moqtail/moqtail/pull/383)), plus DUPLICATE_SUBSCRIPTION / REQUEST_ERROR-on-control-stream / REQUEST_UPDATE-for-PUBLISH / FIN-vs-reset / FETCH-prefix-routing bugs still open at day's end. New live relay `relay18.moqtail.dev` |
| `moq-net` (Pluto TV, stock [[moq-dev]]) | **6/6** both transports | [[steven-riedl|Riedl]]'s client — a positive control isolating the **relay-side prefix-routing** gap shared by imquic, moq-rs and aiomoqt |

[[aiomoqt]] (Python, [[giovanni-marzot|Marzot]]/Vivoh) also stood up a relay role and ran the conformance suite from a pinned moxygen build, cutting **0.11.0rc2** to PyPI during the session.

**Sep-3 — the sweep extends to three more relays.** The same loop (afrind + [[aman-sharma|Aman Sharma]] drive the `moq-test` client, [[steven-riedl|Steven Riedl]]'s stock-[[moq-dev|moq-dev]] `moq-net` client as the positive control) hardened Nokia's, Miniero's, and aiomoqt's relays in one day:

| Relay | Sep-3 movement | Where it still broke |
|---|---|---|
| `moqt-nr` ([[yu-you|Nokia]]) | Patched overnight; **6 clients pass** (Yu You's run) and **12/12 both transports** from Riedl's client; **registered for draft-19** in the runner | afrind's readiness probe: relay **zeroes publisher-priority (200→0) and the contains-last-in-group flag**, delivers the object twice; undefined **Type-0 param** in `REQUEST_OK`; FIN-instead-of-reset; never sends `NAMESPACE_DONE` |
| [[imquic]] (Miniero) | **Prefix-based routing restored**; fixed a pending-publisher-gone hang (SUBSCRIBE for a released namespace now errors instead of hanging) and an unanswered `TRACK_STATUS` | Still **drops publisher priority** across the hop and **doesn't honour `AbsoluteRange`** subscription ranges (Miniero: "look at both tomorrow") |
...
**Aug-31 → Sep-1 progress.** After the retraction, [[mike-english|Mike English]] landed real fixes to moq-rs — *"We definitely weren't handling all of the permutations correctly before"* — and an Aug-31 re-run showed **moq-rs passing Section 1 tests 1, 2 and 3 on both QUIC and WebTransport**, with only **Test 4 (DATAGRAM)** still failing. English then **landed a datagram fix on `draft-18-dev`** (Aug-31 23:35), and afrind's Sep-1 05:08 re-run reported **"Sections 1 and 2 passed for your moq-rs-18 Mike. Section 3 had some troubles, I'm checking"** — so moq-rs now clears the full first two conformance sections, with **Section 3 the current front**. This lifted the automated runner too (see [Current standing](#current-standing): pass 116 → 122 → 125 → 126 across Aug-31 → Sep-1, with moq-rs [#224](https://github.com/cloudflare/moq-rs/pull/224) landing between the two Aug-31 cuts). Meanwhile English began **folding this conformance coverage into the interop runner itself** — [runner PR #103](https://github.com/englishm/moq-interop-runner/pull/103) (new direct-PUBLISH-lifecycle test cases) with companion [#115](https://github.com/englishm/moq-interop-runner/pull/115) (data-plane tests), **both still OPEN**, reusing afrind's portable `moq-test` publisher class (with a floated per-run unique track ID `test-<128-bit-run-id>` to avoid cross-contamination) — discussed in the dedicated **`#moq-interop-runner`** Slack channel. This is the concrete draft-18-readiness picture the [[interim-meetings|Sep-2 virtual interop hackathon]] is meant to harden.
...
| 2026-09-01 00:31:10 | 347 | 126 | 210 | 11 | 231 | 3 | +1 | **matrix −4 → 347; at-target +21 (→231), behind −25 (→113) as impls converge on draft-18 pre-hackathon; stitcher-moq client + moq-go draft-20 ALPN registered** |
| 2026-08-31 04:12:16 | 351 | 125 | 216 | 10 | 210 | 3 | +3 | second Aug-31 cut; new high on the 351-cell matrix; moq-rs #224 landed 02:39 UTC |
| 2026-08-31 00:29:26 | 351 | 122 | 219 | 10 | 210 | 3 | +6 | first Aug-31 cut; recovery off the Aug-30 low begins |
