...
# Activity (May 30 06:00 UTC → May 31 06:00 UTC) — **moq-dev/moq polyglot-split day: Swift + Kotlin + Go all follow the May 30 Python-split template within hours (~14 PRs in 24h, 6-language reference architecture for FFI-distributed protocol stacks); cloudflare/moq-rs PR #169 AuthHook breaks 2-day silence with 8 thibmeu (Cloudflare privacy-pass) review comments; MSF -01 still unfulfilled on Datatracker Day +132 (PR #171 parent-namespace MERGED clears another #146); mailing list + Datatracker + #moq Slack + MoQ Monthly all silent; interop 177/52/124/0 (+2 pass, 29.4%, 13-day cadence, 4-day monotonic uptick to new May high since cadence recovery)**

**TL;DR**:
- **moq-dev/moq executes the Swift/Kotlin/Go polyglot-split template within 24h of PR #1551**. Python split PR #1551 OPEN May 30 05:12 UTC → MERGED 18:08 UTC; **PR #1561 Swift split + Swift-native redesign** (+1810/−489, 32f) OPEN 19:44 UTC → MERGED May 31 02:47 UTC; **PR #1562 Kotlin split** (+704/−181, 19f) OPEN 19:53 UTC → MERGED May 31 02:49 UTC; **PR #1563 Go split + new ergonomic wrapper** (+2333/−309, 33f) OPEN 22:07 UTC → MERGED May 31 03:48 UTC. Plus **PR #1554** ergonomic announced streams (Swift `AsyncSequence` + Kotlin `Flow`), **PR #1559** moq-rs (Python) FFI parity gaps, **PR #1560** wire session stats into the IETF protocol path (closes long-standing gap where IETF connections silently undercounted in dashboard/billing), **PR #1553** count viewers as distinct per-session subscriptions, **PR #1556** drop in-repo cross-language smoke test (moves to new moq-dev/smoke repo to test **published** artifacts not source builds), **PR #1564** declare Cachix cache in flake + document tagged-release caveat, **PR #1558** scrub libiconv from macOS release binaries. ~**14 PRs in 24h**, all kixelated. Six independently-versioned FFI distributions (`moq-ffi` raw bindings + `moq` ergonomic wrapper) × 4 languages (Python/Swift/Kotlin/Go) = **first multi-language FFI release architecture in any tracked MoQ implementation** where ergonomic wrappers float to the latest bindings patch via language-native compatibility ranges (PEP 440 `~=`, SPM `.upToNextMinor`, Maven `[0.2,0.3)`, Go `0.2.x` MVS).
- **cloudflare/moq-rs PR #169 AuthHook breaks 2-day silence with 8 thibmeu review comments** May 30 14:55-15:20 UTC by [[thibault-meunier|Thibault Meunier]] (Cloudflare, privacy-pass author). Substantive design feedback: (1) trait surface might be limiting for **Privacy Pass challenge reply** (cites [draft-ietf-moq-privacy-pass-auth-02 §3.4.5.1](https://datatracker.ietf.org/doc/html/draft-ietf-moq-privacy-pass-auth-02#section-3.4.5.1)); (2) should reference `AUTHORIZATION_TOKEN` parameter directly with its Token Alias / Token Type / Token Value structure; (3) **`&str` vs `&[u8]`** — privacy-pass uses bytes to avoid UTF-8/normalization questions; (4) **draft-18 collapses CLIENT_SETUP/SERVER_SETUP into SETUP** — design should use SETUP consistently; (5) order matters: decode-before-scope or scope-before-knowing-challenge for cross-issuer namespaces. First substantive design engagement on the May 28 AuthHook proposal — privacy-pass perspective forces the trait surface to accommodate **issuer-aware challenge reply** rather than just verifier interface.
- **moq-wg/msf — MSF -01 still unfulfilled on Datatracker May 30-31** (Day +132 since -00 published Jan 19). Wilaw merged **[PR #171](https://github.com/moq-wg/msf/pull/171) MERGED May 30 16:59 UTC** *"Add optional parent namespace field to clone tracks"* (+13/−3, fixes [Issue #146](https://github.com/moq-wg/msf/issues/146) "Clone and track name collisions" — open since pre-May) — clears another long-open issue. **moq-wg/cmsf — Issue #122 "initial text on zapping" attracts mailing-list-style debate** May 30: [[gwendal-simon|Gwendal Simon]] 13:23 UTC argues PR over-explains a simple concept (the issue was raised at the Boulder meeting where Gwendal was notetaker; idea was HESP-style much-more-keyframes track for fast switching); [[suhas-nandakumar|Suhas Nandakumar]] 21:43 UTC reply: *"we can simplify text if needed, but on how do we use aspect, i wonder how much of that should be in this spec? may be an appendix section?"* — first cmsf editor-vs-contributor design pushback the wiki has tracked.
- **Implementations**: **[[moq-dev|moq-dev/moq]]** see above ~14 PRs. **[[moq-rs|cloudflare/moq-rs]]** — **[PR #169](https://github.com/cloudflare/moq-rs/pull/169) 8 thibmeu review comments**, no merge yet; **PR #170 (Manish draft-16 rewrite)** no new commits/comments since May 29 open. **[[openmoq|openmoq/moqx]]** afrind quiet Day +1 since last May 29 14:03 UTC commit — only 2 sync-bot moxygen merges (PR #355 `d1f372e` May 30 10:28 UTC + PR #356 `fe7dd39` May 31 00:03 UTC). **[[quiche-moq|google/quiche moqt]]** 1 commit `e05dcf91` May 30 06:20 UTC *"Fix 1 ClangTidyReadability finding"* — small drive-by; substantive draft-19 prep work (May 28 `c4503a21`+`997d6543` "requests on bidi streams" theme) pauses. **[[mondain|mondain/moqxr]]** `21b791a8` May 30 21:46 UTC *"Fix live object catalog ordering and stop wakeup"* (+53/−7) by Paul Gregoire — first mondain/moqxr commit since pre-May 28 quiet period. **[[moqtail|moqtail/moqtail]]**, **[[moq-js|video-dev/moq-js]]** (PR #72 still OPEN), **[[imquic|meetecho/imquic]]** (PR #27 still OPEN), **[[quiche-moq|birneee/quiche_moq]]**, **[[moqintosh|t-gazzy/Moqintosh]]**, **[[moqlivemock]]**, **Eyevinn/warp-player**, **Eyevinn/moqtransport**, **englishm/moq-interop-runner** all quiet.
- **Interop**: **177 / 52 / 124 / 0** at [2026-05-31 00:47:05 UTC](https://englishm.github.io/moq-interop-runner/results/2026-05-31_004705/report.html) — **+2 pass vs May 30** (50 → 52, 28.2% → 29.4%, **+1.2pp**). **13 consecutive days of daily reports** (May 19-31), extends the longest cadence streak the wiki has tracked. Rolling 5-day band 46-52, trajectory 46 → 49 → 50 → 52 = **4-day monotonic uptick continues**, new May high since cadence recovery. Target still **draft-16** (PR #68 OPEN since May 18). **London hackathon 9 days away**.
...
**[[mondain|Paul Mondain]] CAT token PRs** continue:
...
**[PR #166](https://github.com/moq-wg/msf/pull/166)** (Tobbe's typed-object initDataList design) **MERGED 08:35 UTC** — the structural [[tobbe-einarsson|Torbjörn Einarsson]]-design adoption from May 26 lands in `main`.
...
The headline event: less than 24 hours after [[tobbe-einarsson|Torbjörn Einarsson]]'s May 25 18:44 UTC review comment on [PR #166](https://github.com/moq-wg/msf/pull/166) proposing a typed-object shape for `initDataList`, **wilaw revised the PR at May 26 13:05 UTC** to match:
...
[Issue #153](https://github.com/moq-wg/msf/issues/153) *"`initTrack` does not work"* opened April 10 by [[victor-vasiliev|Victor Vasiliev]] received a **substantive 4-point comment from [[tobbe-einarsson|Torbjörn Einarsson]] on May 23 16:02 UTC** (10 days after the previous comment by suhasHere May 14, which asked to close the issue). Tobbe's comment outlines four open angles before closing:
...
**Carry-forward**: by the [[interim-meetings|London hackathon]], moq-dev/moq is positioned to be **the only implementation able to interop on all three packaging values** (`cmaf`, `loc`, and `mkv` if standardised). The CMSF/Hang unified pipeline gives `hang::Catalog` as a single in-memory IR that can serialize to MSF or Hang catalog formats; the LOC frame layer is now selectable via `Container::Loc` in the catalog. The MKV/WebM exporter (subscribe → write MKV bytes) means **standard players (VLC, MPV, browsers) can consume MoQ broadcasts via a moq-dev/moq exporter** without needing a MoQ-aware player — a structural new deployment shape.
...
[**Paul Gregoire May 20 15:31 UTC**](https://quicdev.slack.com/archives/C046V0QF3CK/p1779291103103049?thread_ts=1779290573.826369): *"I made 'toy' I was messing around with over the weekend, I named mine nearly the same 'moqntosh'; I guess we thought of the same play on words."* — naming-coincidence note; OpenMOQ's [[mondain|Paul Gregoire]] has a private `moqntosh` toy unannounced.
...
The May 18 LOCMAF thread (8 replies) included **Will Law's STS-Sweden talk offer** at 14:46 UTC: *"@Tobbe — I am giving a MSF talk at STS on Thursday. Can I mention locmaf there, or are you already presenting?"* and the prior reply *"You should publish an internet draft and then we can add `packaging = "locmaf"` support to CMSF."* [[tobbe-einarsson|Torbjörn Einarsson]] accepted both at 15:51 / 15:55 UTC: *"I'm not talking at Streaming Tech Sweden, so both Hugo and me think it would be great if you bring it up"* + Mike English's parallel suggestion about MPEG followed with *"I'm not sure this is general enough for MPEG... I'll post something on LinkedIn to reach a bigger audience."* **Net**: LOCMAF gains a second public-conference vehicle (Will Law's STS talk on Thursday May 21) and the *"publish an Internet-Draft"* prescription now has the **MSF/CMSF editor's explicit endorsement** beyond Gwendal Simon's earlier *"expand CMSF"* framing.
...
- Issue #1451 may need to be re-prioritised before [[interim-meetings|London]] if the SGPL camp wants to preserve subscriber-side priority control.
...
Three [[tobbe-einarsson|Torbjörn Einarsson]] PRs merged into Eyevinn/moqlivemock May 17 06:47 → 09:36 UTC, plus the warp-player release at 09:40 UTC:
...
**Headline implication for [[interim-meetings|London interim]]**: the SVC design discussion now has **three positions** to reconcile (TPL, SGPL, layer-type-mixed), not two. Suhas's argument also broadens the ground from *protocol design* (afrind's framing) to *decoder applicability* — a different review surface.
...
**Headline takeaway**: the spec-side resolution path for **non-sequential group sequences** (the same bug class AWS hit at runtime in [[moq-dev|moq-dev]] PR #1413, where CMSF/EML's epoch-based sequences caused ~1s audio underflows) is now **the MOQT Prior Group ID Gap Extension header**. Producers using epoch-based sequences are spec-permitted-with-extension; consumers can switch into gap-aware mode on the wire-level signal.
...
[**PR #84**](https://github.com/Eyevinn/moqlivemock/pull/84) MERGED May 16 06:55 → 10:36 UTC by [[tobbe-einarsson|Torbjörn Einarsson]] (+326/−0, docs-only), *"docs: add DRM section to LOCMAF.md"*. The new `## DRM with LOCMAF` section covers:
...
Continues the post-May-15 LOCMAF tooling sprint by tobbee (PRs #81, #82, #83 May 15 + #84 May 16 = 4 PRs by tobbee in 36 hours, ~+2655 LOC after PR #79 by hugobjoers on May 14). The DRM section completes the **publisher-side documentation surface** for the [[interim-meetings|London interim]] LOCMAF demonstration with warp-player (PR #120 still open).
...
Pre-London preparation push by the wiki owner himself: **[[tobbe-einarsson|Torbjörn Einarsson]] merges 3 PRs May 15 09:36 UTC → 19:37 UTC** consolidating LOCMAF tooling:
...
**Carry-forward**: Either way, **two published WG documents (draft-ietf-moq-transport-18 and draft-ietf-moq-loc-02) now diverge on assigned IANA-style codepoints**. WG editorial coordination between moq-transport and LOC is now a visible item for the [[interim-meetings|London interim (June 9–12)]].
...
Two new posts in the May 6 06:00 UTC → May 7 06:00 UTC window — first `#moq` activity since [[tobbe-einarsson|torbjorn-einarsson]]'s May 5 06:43 CEST moqlivemock+warp-player announcement.
...
- **[PR #82](https://github.com/moq-wg/secure-objects/pull/82) MERGED** May 1 21:05:19 UTC by [[cullen-jennings|fluffy]] ([[suhas-nandakumar]] author, +66/0, *Add padding property for byte boundary alignment*). **Closes Issue #54** (fluffy Nov 2025 — *"add a private header extension for pad to N byte boundary"*). Adds the byte-boundary alignment property promised since November.
- **[PR #77](https://github.com/moq-wg/secure-objects/pull/77) MERGED** May 1 21:06:12 UTC self-merged by [[cullen-jennings|fluffy]] (+50/0, *describe threat model*). Body: *"This most Fixes #49 but making a separate PR to describe the fan out attacks."* **Mostly fixes Issue #49** ("Describe achieved security properties"); the fan-out attack got its own follow-up.
- **PR #78 CLOSED unmerged** May 1 21:29 UTC (the *"DO NOT MERGE YET"* fan-out attack PR), **superseded by PR #86**.
- **[PR #86](https://github.com/moq-wg/secure-objects/pull/86) OPENED + MERGED** in the same 6-minute window May 1 21:27:42 → 21:33:18 UTC by [[cullen-jennings|fluffy]] (opened) → [[suhas-nandakumar]] (merged) (+27/0, *Explain Fan Out Attack*, *"This replaces PR#78 and is part of Fixes #49"*). Closes the fan-out-attack documentation gap.
...
- **[PR #1534](https://github.com/moq-wg/moq-transport/pull/1534) MERGED** May 1 01:11:59 UTC by [[alan-frindell]] (+50/−1, *Add REDIRECT for request errors and established subscriptions*). Lands the redirection mechanism. Adds:
  ...
  - Vasilvv (Apr 27 23:01 UTC) + suhasHere (Apr 29 17:56:52 UTC) approvals before merge. **Closes Issue #1481** ([[cullen-jennings|fluffy]] Feb 9 — *"Do we need a way to move / goaway for individual track"*). Resolves the long-running redirect debate that started Feb 11 between afrind, suhasHere, and others.
...
- **[PR #80](https://github.com/moq-wg/secure-objects/pull/80) MERGED** May 1 03:04:41 UTC ([[suhas-nandakumar]], *Add Publisher Priority to E2E authenticated data*). **Closes Issue #71** ([[cullen-jennings|fluffy]] Mar 16 — *"Add publisher priority to e2e authenticated data"*). Brings publisher priority under the AEAD authentication tag so relays cannot manipulate it.
- **[PR #75](https://github.com/moq-wg/secure-objects/pull/75) MERGED** May 1 03:24:17 UTC ([[cullen-jennings|fluffy]], *guidance on track extentions* [sic]). Adds guidance on how track extensions interact with secure-objects authentication.
- **[PR #76](https://github.com/moq-wg/secure-objects/pull/76) MERGED** May 1 03:24:52 UTC ([[cullen-jennings|fluffy]], *Explain 32-bit object ID nonce limitation*). **Closes Issue #70** ([[alan-frindell]] Mar 15 — *"Is using 32 bits of Object ID in the nonce ok?"*). Documents the implication that publishers must rotate keys before exhausting the 32-bit Object-ID space within a key generation.
...
- **[PR #77](https://github.com/moq-wg/secure-objects/pull/77)** ([[cullen-jennings|fluffy]] Apr 30 — *describe threat model*, mostly fixes #49 — *"Describe achieved security properties with this solution"*).
- **[PR #78](https://github.com/moq-wg/secure-objects/pull/78)** ([[cullen-jennings|fluffy]] Apr 30 — *Describe the fan out attack — DO NOT MERGE YET*). Tracks the multi-subscriber AEAD-collision class.
...
- **[PR #83](https://github.com/moq-wg/secure-objects/pull/83)** ([[cullen-jennings|fluffy]] Apr 30 — *Change the SFRAME ref to point at the RFC*).
- **[PR #84](https://github.com/moq-wg/secure-objects/pull/84)** ([[cullen-jennings|fluffy]] Apr 30 — *Add test vectors in appendix*).
- **[PR #85](https://github.com/moq-wg/secure-objects/pull/85)** ([[cullen-jennings|fluffy]] Apr 30 — *fix up inconsistent dash in end-to-end and hob-by-hop*).

### Other Cullen Apr 30 commit

- **`37a08eb0`** Apr 30 03:31:05 UTC ([[cullen-jennings|fluffy]]): *"Update draft-ietf-moq-secure-objects.md"* — direct main-branch edit (no PR), part of the editorial cleanup.
