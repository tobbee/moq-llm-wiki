...
> **2026-07-08**: **The [[moq-locmaf|LOCMAF]]-into-CMSF question resolves by *reference*, not fold-in — [[tobbe-einarsson|Tobbe]] opens [PR #27](https://github.com/moq-wg/cmsf/pull/27) registering a `locmaf` packaging that points at the standalone [[moq-locmaf|`draft-einarsson-moq-locmaf`]].** **[PR #27](https://github.com/moq-wg/cmsf/pull/27)** *"Register locmaf packaging referencing draft-einarsson-moq-locmaf"* (tobbee, July 7 13:04 UTC, +74/−1, OPEN) adds LOCMAF as a CMSF packaging **by normative reference to the standalone [[moq-locmaf|LOCMAF -01]] draft** rather than copying the format text into CMSF — the approach **[[will-law|wilaw]] suggested on [Issue #24](https://github.com/moq-wg/cmsf/issues/24)**. This is the concrete resolution of the June-28 Issue #24 (*"Incorporate LOCMAF as a packaging mode in CMSF"*): CMSF now spans `cmaf` / `locmaf` / `loc` packagings, but `locmaf` is defined *elsewhere* (Eyevinn's draft + [Eyevinn/locmaf](https://github.com/Eyevinn/locmaf) reference repo) and CMSF just signals it in the catalog (`packaging: "locmaf"` + `locmafVersion`). So LOCMAF **keeps its independent individual-draft path** (contrary to the earlier "fold in and retire the standalone" framing) — CMSF gains the mode, LOCMAF stays sovereign over the format. Ties to [Eyevinn/locmaf PR #2](https://github.com/Eyevinn/locmaf/pull/2) (mp4ff v0.53.0 bump, July 7). See [[moq-locmaf]], [[discussions-2026-07]].
>
> **2026-07-01**: **[[will-law|Will Law]] opens a CMSF catalog-URL PR + a fragment-identifier issue (June 30) — the CMSF editorial front reactivates alongside his parallel [[moq-msf|MSF]] initData work.** **[PR #26](https://github.com/moq-wg/cmsf/pull/26)** *"Document URL usage for CMSF catalogs"* (+7/−0, June 30 13:19 UTC, OPEN) documents how catalog URLs are used with CMSF, and companion **[Issue #25](https://github.com/moq-wg/cmsf/issues/25)** *"Clarify which fragment type identifier to use with CMSF"* (June 30 13:19 UTC, OPEN) picks up the **MSF/CMSF URI fragment-identifier question** Gwendal raised in the London Day-2 burst ([msf #181](https://github.com/moq-wg/msf/issues/181)). These land the same day as wilaw's [[moq-msf|MSF]] PRs [#186](https://github.com/moq-wg/msf/pull/186) (dual Track+Object initData) / [#187](https://github.com/moq-wg/msf/pull/187) (track cloning + delta rules) — a coordinated media-format editorial push across both specs while [[tobbe-einarsson|Tobbe]]'s [Issue #24](https://github.com/moq-wg/cmsf/issues/24) (fold [[moq-locmaf|LOCMAF]] in as a `locmaf` packaging) stays OPEN. See [[moq-msf]], [[moq-locmaf]], [[discussions-2026-06]].
>
> **2026-06-29**: **[[tobbe-einarsson|Torbjörn Einarsson]] opens [Issue #24](https://github.com/moq-wg/cmsf/issues/24) to fold [[moq-locmaf|LOCMAF]] into CMSF as a third packaging mode — the first formal step on the London interim-11 agreement.** Filed June 28, **[Issue #24](https://github.com/moq-wg/cmsf/issues/24)** *"Incorporate LOCMAF as a packaging mode in CMSF"* turns the **[[interim-meetings|London interim-11]] (Day-2, June 12) consensus** — that LOCMAF (`draft-einarsson-moq-locmaf-00`, Einarsson / Björs) should **become part of CMSF rather than proceed as a standalone draft** — into a tracked work item with a concrete design. It proposes a **`locmaf` packaging** sitting between the two CMSF already spans: `cmaf` (full CMAF chunks → MSE passthrough), **`locmaf`** (tagged fields + unmodified samples → receiver reconstructs the CMAF chunk for MSE, or extracts samples for WebCodecs), and `loc` (codec-elementary frames → WebCodecs). The argument: LOCMAF is **end-to-end** (relays forward the MOQT Object payload unchanged; only the encoder produces LOCMAF, only the receiver expands it) and reconstructs the *same* CMAF chunk a `cmaf` track would carry, so it reuses CMSF's existing catalog/init machinery while dropping steady-state per-object overhead to a couple of bytes (comparable to [[moq-loc|LOC]], including Common Encryption). This is the natural landing of wilaw's June-9 **dual Track+Object init** proposal and the June-3 initData-carriage thread — and the path to **retiring the standalone LOCMAF individual draft** in favor of a defined CMSF mode. See [[moq-locmaf]], [[discussions-2026-06]], [[interim-meetings]].
>
> **2026-06-10**: **Will Law previews a dual Track+Object initData proposal for the London CMSF slot.** In the June-9 16:07 CEST [[moq-locmaf|LOCMAF]] Slack thread, **[[will-law|wilaw]]**: *"In the CMSF slides for London, I propose that we create **both Track and Object properties to carry Init Data**. This would solve the two problems of mid-track changes and synchronization."* This is the lead resolution of the [moq-msf Issue #153](https://github.com/moq-wg/msf/issues/153) initTrack design problem the wiki has tracked since the June-3 thread (afrind's *"init data as a track property"* / *"only publish the init subgroup in groups where it changed"* vs wilaw's *"we do not allow track init properties to change once publish has begun"*). The dual-property answer keeps a stable **Track** property for steady-state init while adding an **Object** property for synchronized mid-track changes (DASH-period switches, DRM key rotation) — and is the natural home for the catalog-referenced init that [[moq-msf|MSF]] -01 introduced and [[moq-locmaf|LOCMAF]] 0.2 relies on. Now the lead substantive item for the **June-12 Day-2 MSF/CMSF Will Law slot**. See [[moq-locmaf]], [[discussions-2026-06]], [[interim-meetings]].
>
> **2026-06-07**: **Tobbe's CUE catalog validator finds CMSF draft-01 bugs → [PR #23](https://github.com/moq-wg/cmsf/pull/23) OPEN June 4** *"Fix version examples and minor typos in draft-01"* (+5/−5). Companion to [moq-wg/msf PR #177](https://github.com/moq-wg/msf/pull/177): [[tobbe-einarsson|Torbjörn Einarsson]]'s [MSF/CMSF catalog validator](https://github.com/Eyevinn/msf-catalog-validator) (announced on `#moq` June 5, validates *strictly against draft-01 definitions*) surfaced wrong `version` example values + typos in CMSF -01. Pure editorial fixes for a future -02; the validator additionally covers [[moq-locmaf|LOCMAF]] packaging, exercising the CMSF/LOCMAF init-reference split that came out of the [[discussions-2026-06|June-3 initData thread]]. See [[moq-msf]] for the validator detail and [[interim-meetings]] — CMSF stays on the London Day-2 Will Law slot.
>
...
- **Issue #24** (Jun 28, [[tobbe-einarsson|tobbee]]) - *"Incorporate LOCMAF as a packaging mode in CMSF"* - tracks the London interim-11 agreement to fold [[moq-locmaf|LOCMAF]] into CMSF as a third `locmaf` packaging (between `cmaf` and `loc`) rather than ship it as a standalone individual draft.
