...
- **LOCMAF folding into CMSF**: the [[interim-meetings|London interim-11]] consensus is that [[moq-locmaf|LOCMAF]] should become part of CMSF rather than a standalone draft — a proposed third `locmaf` packaging mode between `cmaf` (full CMAF chunks → MSE passthrough) and `loc` (codec-elementary frames → WebCodecs), carrying tagged fields plus unmodified samples so the receiver reconstructs the same CMAF chunk while dropping steady-state overhead to a couple of bytes.
