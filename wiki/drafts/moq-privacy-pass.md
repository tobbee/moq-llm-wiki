...
> **2026-07-06**: **The auth workstream lands its first *merged spec text* since the July-1 triage — [[thibault-meunier|Thibault Meunier]]'s [PR #20](https://github.com/moq-wg/privacy-pass/pull/20) *"Key considerations"* MERGES July 5 21:42 UTC (by [[suhas-nandakumar|Suhas]], +44/−5), adding a "Deployment considerations" appendix on key-material retrieval.** The PR (thibmeu, created July 5 07:46 UTC, merged by Suhas) adds a new **`# Deployment considerations`** back-section to `draft-ietf-moq-privacy-pass-auth.md` covering two key-bootstrap problems the draft had left explicitly undefined: (1) **fetching key material for the bootstrap issuer** — Clients *may* reuse issuers exposed over HTTP (**RFC 9577**), and *should* reduce susceptibility to **partitioning** (which erodes the Privacy Pass privacy guarantee, RFC 9576 §6.2) via consistency **mirrors** (the `draft-ietf-privacypass-consistency-mirror` I-D) or transparency mechanisms (**KEYTRANS** = `draft-ietf-keytrans-architecture`, or **SCITT** = RFC 9943); and (2) **fetching key material for the relay *reverse* issuer** — the tie-back to the **relay-as-issuer reverse-flow mode Suhas raised in [#19](https://github.com/moq-wg/privacy-pass/issues/19)** (July 1): key material can be pre-established (shipped with app install / webpage load / device setup) or fetched on the fly via HTTP or a MoQ API. The PR also reformats the entity list (Client / Relay / Issuer / Attester) as a definition list, renames "Change Log" → "Changelog", and adds references to thibmeu's own **`draft-meunier-privacypass-reverse-flow`** plus the mirror / KEYTRANS / SCITT I-Ds. This is the concrete spec-text follow-through on the **July-1 Suhas triage** (#19/#18/#16) and the first privacy-pass *merge* since thibmeu's June issue burst. No new datatracker revision yet (still -02; the merge is on `main`, un-cut). Lands the morning of **[[interim-meetings|interim-2026-moq-18]] (July 6, 16:30 UTC)**. See [[moq-c4m]], [[moq-transport]], [[discussions-2026-07]].
>
