...
- **LOC private-properties encoding is underspecified — and it has now caused a concrete cross-implementation interop mismatch**: the draft does not say how private properties behave when absent — omit the block entirely vs write a zero-count varint (a `0x00` prefix on the frame). Surfaced under live two-impl media interop; [[imquic]] resolved it implementation-side by writing an explicit payload prefix for LOC private objects (its own [PR #29](https://github.com/meetecho/imquic/pull/29)). On **2026-08-25** [[lorenzo-miniero|Miniero]] reported (Slack `#moq`) that after updating imquic to loc-04 he got media to work end-to-end against Jordi Cenzano's `moq-encoder-player` **except** that moq-encoder-player omits the private-properties block entirely while imquic expects the leading `0x00` count byte — so his receiver mis-parses unless that block is skipped. He reads **§3.1.3 (Private Properties)** as ambiguous (whether the check is "first byte is present as a zero count" vs "first byte is `0xA` ⇒ private properties present") and asked [[suhas-nandakumar|Suhas Nandakumar]] how to handle proper interop even when [[moq-secure-objects|Secure Objects]] are not in use. **On 2026-08-26 Suhas (a LOC author) answered on-thread**: private properties are **not** specific to Secure Objects — they are ordinary payload properties encoded as standard **Key-Value-Pairs**, so the recommended way to signal *no* private properties is a **zero-length KVP** (a type + length), **not** omission and **not** a bare `0x00` count byte. Working it through with Miniero, the object-payload prefix for an empty private-properties set landed at **`0xA 0x00`** (type `0xA`, length `0`) before the media data — Suhas to confirm once he can re-check. So both implementations were slightly off (imquic wrote a bare `0x00`; moq-encoder-player omitted the block); Miniero will align imquic and then retry imquic ↔ moq-encoder-player media interop. (A second-direction issue surfaced too: moq-encoder-player rejects the video imquic sends.) The §3.1.3 ambiguity now has an author-level reading but no draft text yet — still an open spec question, with a reproducing pair.
