...
> **2026-07-08**: **Post-`-03` cleanup: a property-type collision gets fixed in moq-transport and the LOC issue tracker is swept.** The day after the -03 cut, **[[mo-zanaty|Mo Zanaty]] opens+closes [moq-transport Issue #1806](https://github.com/moq-wg/moq-transport/issues/1806)** *"Subgroup Delivery Timeout and Timestamp have same value 0x06"* and lands the fix as **[moq-transport PR #1807](https://github.com/moq-wg/moq-transport/pull/1807)** *"Fix LOC property type values"* (July 7 23:27 UTC, +4/−3) — realigning the LOC property type IDs (TIMESTAMP, VIDEO_FRAME_MARKING, VIDEO_CONFIG, AUDIO_CONFIG) so the new AUDIO_CONFIG from -03 and the existing TIMESTAMP no longer collide with moqt code points (the long-running §15.8 property-ID coordination the wiki has tracked since [Issue #20](https://github.com/moq-wg/loc/issues/20)). In parallel, the **LOC repo tracker was swept clean July 7 evening** (kixelated + yuanchao-chris): **six issues closed as COMPLETED** — [#21](https://github.com/moq-wg/loc/issues/21) (audio config, answered by -03 PR #24), [#20](https://github.com/moq-wg/loc/issues/20) (LOC-02 property-type collision), [#15](https://github.com/moq-wg/loc/issues/15) (Video Frame Marking vi64 ambiguity), [#13](https://github.com/moq-wg/loc/issues/13) (duplicate 0x06 for Timestamp/Audio Level), [#10](https://github.com/moq-wg/loc/issues/10) (moqt-draft17 vs loc-01 collision), [#5](https://github.com/moq-wg/loc/issues/5) (move LOC header metadata into payload). Only **[Issue #19](https://github.com/moq-wg/loc/issues/19)** *"How do you encode LOC Private Properties?"* stays OPEN — the last live LOC design question, the same private-properties encoding ambiguity Lorenzo Miniero raised at the London hackathon. See [[moq-transport]], [[discussions-2026-07]].
>
