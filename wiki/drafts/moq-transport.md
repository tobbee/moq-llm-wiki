...
- **Should `NAMESPACE` carry parameters like `PUBLISH_NAMESPACE`?**: [[luke-curley|Luke Curley]]'s [issue #1800](https://github.com/moq-wg/moq-transport/issues/1800), which [[ian-swett|Ian Swett]] scheduled for the **Aug-24 virtual interim** (*"I'd like to discuss this at Monday's interim on 8/24"*). The adjacent [issue #1854](https://github.com/moq-wg/moq-transport/issues/1854) (*"to PUBLISH_NAMESPACE or to not PUBLISH_NAMESPACE"*, also Curley) was labeled **BLOCKED** and self-assigned by afrind on Aug-20 — the PUBLISH_NAMESPACE debate is now explicitly parked pending that interim. See [[interim-meetings]].
